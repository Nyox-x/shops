...
class SuperCat(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)  # 编号
    cat_name = db.Column(db.String(100))  # 一级分类名称
...
class SubCat(db.Model):
    ...
    addtime = db.Column(db.DateTime, index=True, default=datetime.now)  # 添加时间
    super_cat_id = db.Column(db.Integer, db.ForeignKey('supercat.id'))  # 所属一级分类
...
class Goods(db.Model):
    ...
    # 设置外键
    supercat_id = db.Column(db.Integer, db.ForeignKey('supercat.id'))  # 所属一级分类
    subcat_id = db.Column(db.Integer, db.ForeignKey('subcat.id'))  # 所属二级分类
    addtime = db.Column(db.DateTime, index=True, default=datetime.now)  # 添加时间
