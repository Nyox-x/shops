...
def subcat_add():
    ...
    if request.method == 'POST':
        ...
        if subcat :
            return "<script>alert('该二级分类已经存在');history.go(-1);</script>"
        # 组织数据
        data = SubCat(
    ...
    supercat = SuperCat.query.all()  # 获取一级分类信息
